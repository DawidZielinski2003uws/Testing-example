...
class PersonController {
  ...
  public void addPaperToTutor(@PathVariable Long Id, @RequestBody PaperDTO paperDTO) {
    personService.addPaperToTutor(Id, paperDTO);
  }

  @DeleteMapping("/{personId}/papers/{paperId}")
    @Operation(summary = "Remove paper from tutor")
    public void removePaperFromTutor(@PathVariable Long personId, @PathVariable Long paperId) {
    personService.removePaperFromTutor(personId, paperId);
  }
}
